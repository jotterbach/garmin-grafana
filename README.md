...
2. `compose.yml` is already checked into this repository and reads all secrets from a `.env` file next to it (via Compose's automatic `${VAR}` interpolation), rather than holding them inline -- that keeps real credentials out of git entirely. Copy the template with `cp .env.example .env`, then fill in your real values (`INFLUXDB_USER`, `INFLUXDB_PASSWORD`, and optionally `GARMINCONNECT_EMAIL`/`GARMINCONNECT_BASE64_PASSWORD`/`GRAFANA_ADMIN_PASSWORD`) -- see the comments in `.env.example` for what each one does. `.env` is gitignored, so it's safe to put real secrets there. If you don't want to pass Garmin Connect credentials this way, just leave those two `.env` lines empty and you'll be prompted for them interactively instead (see step 4).

3. Please note the Garmin Connect password must be encoded with [Base64](http://base64encode.org/) when set via the `GARMINCONNECT_BASE64_PASSWORD` variable in `.env` -- the script decodes it when required. If you set `GARMINCONNECT_EMAIL` and `GARMINCONNECT_BASE64_PASSWORD` and do not have two factor authentication (via SMS or email), you can directly jump to `step 5`. If you are in mainland China and use Garmin-cn account you need to set `GARMINCONNECT_IS_CN=True` in `compose.yml`. You can also select what data you want to fetch with the `FETCH_SELECTION` variable in `compose.yml`.
...
✅ By default, the pulled FIT files are not stored as files to save storage space during import (an in-memory IO buffer is used instead). If you want to keep the FIT files downloaded during the import for future use in `Strava` or any other application where FIT files are supported for import, set `KEEP_FIT_FILES=True` in your `.env` file (see `.env.example`) -- the `garmin-fetch-data` service's volume mount already maps a `fit_filestore` folder for this, no compose.yml editing needed. Create that folder next to your compose file with `mkdir fit_filestore` and change its ownership with `chown 1000:1000 fit_filestore` before starting the container. You will see the FIT files for your activities appear inside this `fit_filestore` folder once the script starts running. If you'd rather store them somewhere other than `./fit_filestore` (e.g. a larger disk), set `FIT_FILESTORE_HOST_PATH` in `.env` to an absolute path instead (same ownership requirement applies).
...
Since this fork's `garmin-fetch-data` service builds from this repository's own source (`build: .`) rather than pulling a published image, updating means pulling the latest source and rebuilding: `git pull`, then `docker compose up -d --build` (`docker compose pull` only refreshes the `influxdb` and `grafana` images, not `garmin-fetch-data`). Please verify everything is running correctly by checking the logs with `docker compose logs --follow`
