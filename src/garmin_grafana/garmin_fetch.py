...
def get_daily_stats(date_str):
    ...
    if stats_json["wellnessStartTimeGmt"] and datetime.strptime(date_str, "%Y-%m-%d") < datetime.today():
        timestamp = pytz.timezone("UTC").localize(
            datetime.strptime(stats_json["wellnessStartTimeGmt"], "%Y-%m-%dT%H:%M:%S.%f")
        )
        fields = {
...
def get_last_sync():
    ...
    points_list.extend(
        build_timestamped_point(
            "DeviceSync",
            timestamp,
            {"imageUrl": sync_data.get("imageUrl"), "Device_Name": GARMIN_DEVICENAME},
            device_name=GARMIN_DEVICENAME,
            database_name=INFLUXDB_DATABASE,
        )
    )
...
def get_sleep_data(date_str):
    ...
    if sleep_levels_intraday:
        for entry in sleep_levels_intraday:
            if (
                entry.get("activityLevel") or entry.get("activityLevel") == 0
            ):  # Include 0 for Deepsleep but not None - Refer to issue #43
                timestamp = pytz.timezone("UTC").localize(datetime.strptime(entry["startGMT"], "%Y-%m-%dT%H:%M:%S.%f"))
                fields = {
    ...
    if sleep_spo2_intraday:
        for entry in sleep_spo2_intraday:
            if entry.get("spo2Reading"):
                timestamp = pytz.timezone("UTC").localize(
                    datetime.strptime(entry["epochTimestamp"], "%Y-%m-%dT%H:%M:%S.%f")
                )
                points_list.extend(
                    build_timestamped_point(
                        "SleepIntraday",
...
def get_intraday_hrv(date_str):
    ...
    for entry in hrv_list:
        if entry.get("hrvValue"):
            timestamp = pytz.timezone("UTC").localize(
                datetime.strptime(entry["readingTimeGMT"], "%Y-%m-%dT%H:%M:%S.%f")
            )
            points_list.extend(
                build_timestamped_point(
                    "HRV_Intraday",
...
def get_activity_summary(date_str):
    ...
    if ACTIVITY_TYPE_FILTER:
        activity_list = [
            ...
            if (a.get("activityType") or {}).get("typeKey", "Unknown").lower() in ACTIVITY_TYPE_FILTER
        ]
        logging.info(
            f"ACTIVITY_TYPE_FILTER active: kept {len(activity_list)} activities matching {ACTIVITY_TYPE_FILTER}"
        )
    for activity in activity_list:
        activity_type_key = (activity.get("activityType") or {}).get("typeKey", "Unknown")
        if (
            activity.get("hasPolyline") or ALWAYS_PROCESS_FIT_FILES
        ):  # will process FIT files lacking GPS data if ALWAYS_PROCESS_FIT_FILES is set to True
            if not activity.get("hasPolyline"):
                logging.warning(
                    f"Activity ID {activity.get('activityId')} got no GPS data - yet, activity FIT file data will be processed as ALWAYS_PROCESS_FIT_FILES is on"
                )
        ...
        if "startTimeGMT" in activity:  # "startTimeGMT" should be available for all activities (fix #13)
            ...
            activity_start_time = datetime.strptime(activity["startTimeGMT"], "%Y-%m-%d %H:%M:%S").replace(
                tzinfo=pytz.UTC
            )
            activity_selector = activity_start_time.strftime("%Y%m%dT%H%M%SUTC-") + (
                activity.get("activityType") or {}
            ).get("typeKey", "Unknown")
            ...
            points_list.extend(
                build_timestamped_point(
                    ...
                    activity_start_time,
                    {
                        ...
                        if activity.get("elapsedDuration")
            ...
            end_time = activity_start_time + timedelta(
                seconds=int(activity.get("elapsedDuration", activity.get("duration", 0)))
            )
            points_list.extend(
                build_timestamped_point(
                    "ActivitySummary",
...
def purge_existing_strength_exercise_sets(activity_id):
    ...
    try:
        ...
        logging.warning(f"Failed to purge existing StrengthExerciseSet series for activity {activity_id}: {err}")
        return False
    ...
    for activity_id, activity_info in strength_activity_id_dict.items():
        ...
        try:
            ...
            for exercise in exercises:
                ...
                if start_ts:
                    set_time = (
                        datetime.strptime(start_ts.split(".")[0], "%Y-%m-%dT%H:%M:%S")
...
def fetch_activity_GPS(activityIDdict):  # Uses FIT file by default, falls back to TCX
    ...
    for activityID in activityIDdict.keys():
        ...
        try:
            ...
            with zipfile.ZipFile(zip_buffer) as zip_ref:
                ...
                if not fit_filename:
                    raise FileNotFoundError(
                        f"No FIT file found in the downloaded zip archive for Activity ID {activityID}"
                    )
                else:
                    ...
                    if len(all_records_list) == 0:
                        raise FileNotFoundError(
                            f"No records found in FIT file for Activity ID {activityID} - Discarding FIT file"
                        )
                    else:
                        ...
                        if parsed_record.get("timestamp"):
                            point = {
                                ...
                                "tags": {
                                    ...
                                    "Database_Name": INFLUXDB_DATABASE,
                                    "ActivityID": activityID,
                                    "ActivitySelector": activity_start_time.strftime("%Y%m%dT%H%M%SUTC-")
                                    + activity_type,
                                },
                                "fields": {
                                    "ActivityName": activity_type,
                                    "Activity_ID": activityID,
                                    "Latitude": int(parsed_record["position_lat"]) * (180 / 2**31)
                                    if parsed_record.get("position_lat")
                                    else None,
                                    "Longitude": int(parsed_record["position_long"]) * (180 / 2**31)
                                    if parsed_record.get("position_long")
                                    else None,
                                    "Altitude": parsed_record.get("enhanced_altitude", None)
                                    or parsed_record.get("altitude", None),
                                    "Distance": parsed_record.get("distance", None),
                                    "DurationSeconds": (
                                        parsed_record["timestamp"].replace(tzinfo=pytz.UTC) - activity_start_time
                                    ).total_seconds(),
                                    "HeartRate": float(parsed_record.get("heart_rate", None))
                                    if parsed_record.get("heart_rate", None)
                                    else None,
                                    "Speed": parsed_record.get("enhanced_speed", None)
                                    or parsed_record.get("speed", None),
                                    "GradeAdjustedSpeed": (parsed_record.get(140) / 1000.0)
                                    if parsed_record.get(140)
                                    else None,
                                    "RunningEfficiency": (
                                        (parsed_record.get(140) / 1000.0) / parsed_record.get("heart_rate")
                                    )
                                    if (parsed_record.get(140) and parsed_record.get("heart_rate"))
                                    else None,
                    ...
                    for session_record in all_sessions_list:
                        if session_record.get("start_time") or session_record.get("timestamp"):
                            point = {
                                ...
                                "time": session_record["start_time"].replace(tzinfo=pytz.UTC).isoformat()
                                or session_record["timestamp"].replace(tzinfo=pytz.UTC).isoformat(),
                                "tags": {
                                    "Device": GARMIN_DEVICENAME,
                                    "Database_Name": INFLUXDB_DATABASE,
                                    "ActivityID": activityID,
                                ...
                                "fields": {
                                    "Index": (
                                        int(v) if str(v := session_record.get("message_index", -1)).isdigit() else -1
                                    )
                                    + 1,
                                    "ActivityName": activity_type,
                                    "Activity_ID": activityID,
                                    "Sport": str(
                                        session_record.get("sport", None)
                                    ),  # Avoid partial write error 400 see #152#issuecomment-3084539416
                    ...
                    for length_record in all_lengths_list:
                        if length_record.get("start_time") or length_record.get("timestamp"):
                            point = {
                                ...
                                "time": length_record["start_time"].replace(tzinfo=pytz.UTC).isoformat()
                                or length_record["timestamp"].replace(tzinfo=pytz.UTC).isoformat(),
                                "tags": {
                                    "Device": GARMIN_DEVICENAME,
                                    "Database_Name": INFLUXDB_DATABASE,
                                    "ActivityID": activityID,
                    ...
                    for lap_record in all_laps_list:
                        if lap_record.get("start_time") or lap_record.get("timestamp"):
                            point = {
                                ...
                                "time": lap_record["start_time"].replace(tzinfo=pytz.UTC).isoformat()
                                or lap_record["timestamp"].replace(tzinfo=pytz.UTC).isoformat(),
                                "tags": {
                                    "Device": GARMIN_DEVICENAME,
                                    "Database_Name": INFLUXDB_DATABASE,
                                    "ActivityID": activityID,
                    ...
                    if "cycling_dynamics" in FETCH_SELECTION:
                        cycling_point = _build_cycling_dynamics_point(
                            all_records_list, all_sessions_list, activityID, activity_type, activity_start_time
                        )
                        if cycling_point:
                            points_list.append(cycling_point)
                            logging.info(
                                f"Activity ID {activityID}: CyclingDynamics point added ({len(cycling_point['fields']) - 2} metrics)"
                            )
            ...
            try:
                tcx_file_data = garmin_obj.download_activity(
                    activityID, dl_fmt=garmin_obj.ActivityDownloadFormat.TCX
                ).decode("UTF-8")
                root = ET.fromstring(tcx_file_data)
                if KEEP_FIT_FILES:
                    os.makedirs(FIT_FILE_STORAGE_LOCATION, exist_ok=True)
                    activity_start_time = datetime.fromisoformat(
                        root.findall("tcx:Activities/tcx:Activity", ns)[0].find("tcx:Id", ns).text.strip("Z")
                    )
            ...
            for activity in root.findall("tcx:Activities/tcx:Activity", ns):
                ...
                for lap in activity.findall("tcx:Lap", ns):
                    # Value unused, but parsing it validates/fails fast on a
                    # malformed Lap StartTime before processing trackpoints --
                    # preserved as a bare expression rather than dropped
                    # entirely (see #12's lint-cleanup PR).
                    datetime.fromisoformat(lap.attrib.get("StartTime").strip("Z"))
                    for tp in lap.findall(".//tcx:Trackpoint", ns):
                        time_obj = datetime.fromisoformat(
                            tp.findtext("tcx:Time", default=None, namespaces=ns).strip("Z")
                        )
...
def get_lactate_threshold(date_str):
    ...
    for ltsport in LACTATE_THRESHOLD_SPORTS:
        ...
        endpoints[f"SpeedThreshold_{ltsport}"] = (
            f"/biometric-service/stats/lactateThresholdSpeed/range/{date_str}/{date_str}",
            params,
        )
        endpoints[f"HeartRateThreshold_{ltsport}"] = (
            f"/biometric-service/stats/lactateThresholdHeartRate/range/{date_str}/{date_str}",
            params,
        )
    ...
    if ts_training_data_all:
        for device_id, ts_dict in ts_training_data_all.items():
            ...
            data_fields = {
                ...
                "dailyAcuteChronicWorkloadRatio": (ts_dict.get("acuteTrainingLoadDTO") or {}).get(
    ...
    if tr_list_all:
        for tr_dict in tr_list_all:
            ...
            if (not all(value is None for value in data_fields.values())) and tr_dict.get("timestamp"):
                timestamp = pytz.timezone("UTC").localize(
                    datetime.strptime(tr_dict["timestamp"], "%Y-%m-%dT%H:%M:%S.%f")
                )
                points = build_timestamped_point(
                    "TrainingReadiness",
                    timestamp,
        ...
        for bp_measurement in bp_list:
            ...
            if not all(value is None for value in data_fields.values()) and "measurementTimestampGMT" in bp_measurement:
                timestamp = pytz.UTC.localize(
                    datetime.strptime(bp_measurement["measurementTimestampGMT"], "%Y-%m-%dT%H:%M:%S.%f")
                )
                points_list.extend(
                    build_timestamped_point(
                        "BloodPressure",
    ...
    if len(si_all.get("solarDailyDataDTOs", [])) > 0:
        ...
        for si_measurement in si_list:
            ...
            if not all(value is None for value in data_fields.values()) and "readingTimestampGmt" in si_measurement:
                timestamp = pytz.UTC.localize(
                    datetime.strptime(si_measurement["readingTimestampGmt"], "%Y-%m-%dT%H:%M:%S.%f")
                )
                points_list.extend(
                    build_timestamped_point(
                        "SolarIntensity",
...
def _request_intraday_refresh(date_str):
    ...
    """
    if not (
        REQUEST_INTRADAY_DATA_REFRESH
        and (
            datetime.strptime(date_str, "%Y-%m-%d")
            <= (datetime.today() - timedelta(days=IGNORE_INTRADAY_DATA_REFRESH_DAYS))
        )
    ):
        return True

    data_refresh_response = garmin_obj.connectapi(
        f"wellness-service/wellness/epoch/request/{date_str}", method="POST"
    ).get("status", "Unknown")
        ...
        while repeat_loop:
            try:
                ...
                if is_500_error:
                    ...
                    logging.error(
                        f"HTTP 500 error ({consecutive_500_errors}/{MAX_CONSECUTIVE_500_ERRORS}) for date {current_date}: {err}"
                    )
                    ...
                    logging.error(err)
                    logging.info(
                        f"HTTP Error (non-500) : Failed to fetch one or more metrics - skipping date {current_date}"
                    )
                    logging.info(f"Waiting : for {RATE_LIMIT_CALLS_SECONDS} seconds")
                    time.sleep(RATE_LIMIT_CALLS_SECONDS)
                    repeat_loop = False
...
def _maybe_sync_once(last_influxdb_sync_time_UTC, local_timediff):
    ...
    if last_influxdb_sync_time_UTC < last_watch_sync_time_UTC:
        ...
        )  # Using local dates for deciding which dates to fetch in current iteration (see issue #25)
